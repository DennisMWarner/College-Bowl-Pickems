...
namespace BowlPickems.Repositories
{
  public class TeamsRepository
  {
    ...
    {
      string sql = @"
            INSERT INTO teams(name, abbName, priColor, secColor,  conference, confRecord, overallRec,  streak, gameId)
            VALUES(@Name, @AbbName, @PriColor, @SecColor, @Conference, @ConfRecord, @OverallRec,  @Streak, @GameId); 
    ...
    {
      {
        string sql = "UPDATE teams SET name = @name, abbName=@AbbName, priColor=@PriColor, secColor=@SecColor,  confRecord=@ConfRecord, overallRec=@OverallRec, mascotName=@MascotName, conference=@Conference,  gameId=@GameId, streak=@Streak  WHERE id = @Id";
        int affectedRows = _db.Execute(sql, teamToUpdate);
        return teamToUpdate;
      }
    }
