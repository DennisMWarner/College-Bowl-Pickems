...
namespace BowlPickems.Repositories
{
  public class GamesRepository
  {
    ...
    {
      string sql = @"
            INSERT INTO games(name, gameDate, loc, time, network, sponsor, bowlUrl, points)
            VALUES(@Name, @GameDate, @Loc, @Time, @Network, @Sponsor, @BowlUrl, @points); 
    ...
    {
      {
        string sql = "UPDATE Games SET name = @name, sponsor=@Sponsor,bowlUrl=@BowlUrl, gameDate=@GameDate, loc=@Loc, time=@Time, network=@Network, points=@Points WHERE id = @Id";
